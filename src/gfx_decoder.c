...
struct gfx_decoder* gfx_decoder_create(char const* head_path, char const* graph_path, char const* dict_path) {
	...
	for (int i = 1; i < decoder->head_size-1; i++) {
		if (decoder->head_buffer[i] != HEAD_EMPTY_SENTINEL) {
			decoder->head_bytes[prev_index] = decoder->head_buffer[i] - decoder->head_buffer[prev_index];
			prev_index = i;
		}
	}
...
int gfx_decoder_decode_sized_chunk(struct gfx_decoder* decoder, int chunk_id, uint8_t* dst) {
  ...
  CAL_HuffExpand(compressed_chunk, dst, size, decoder->dict_buffer, decoder->head_bytes[chunk_id]);
	return size;
}

int gfx_decoder_decode_unsized_chunk(struct gfx_decoder* decoder, int chunk_id, int size, uint8_t* dst) {
  uint32_t offset = decoder->head_buffer[chunk_id];
